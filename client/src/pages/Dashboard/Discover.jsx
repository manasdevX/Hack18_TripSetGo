...
function TripCard({ trip }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div style={{ height: 120, borderRadius: 'var(--radius-md)', background: 'var(--gradient-primary)', display: 'flex', alignItems: 'flex-end', padding: '1rem', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.3)' }} />
        <div style={{ position: 'relative', zIndex: 1 }}>
          <p style={{ fontWeight: 800, fontSize: '1.125rem' }}>{trip.destination}</p>
          <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '0.8rem' }}>from {trip.source}</p>
        </div>
      </div>
      {/* Content */}
      <div>
  ...
  return (
    <div className="page-enter">
      ...
      <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem', flexWrap: 'wrap' }}>
        ...
        <select className="input" style={{ width: 160 }} value={filters.sortBy} onChange={e => dispatch(setFilters({ sortBy: e.target.value }))}>
