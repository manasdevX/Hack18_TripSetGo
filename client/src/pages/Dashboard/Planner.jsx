...
import { Sparkles, Send, RotateCcw, DollarSign, Train, Hotel, Utensils, CalendarDays, Lightbulb } from 'lucide-react'
import {
  generatePlan, updateForm, resetPlan,
...
import { selectPlanner } from '@/features/planner/plannerSlice'
import Button from '@/components/common/Button'
import Input from '@/components/common/Input'
import Loader from '@/components/common/Loader'

const GROUP_TYPES = ['solo', 'couple', 'family', 'friends', 'business']
const PREFERENCES = ['beach', 'mountains', 'culture', 'food', 'adventure', 'nightlife', 'wildlife', 'relaxation', 'shopping', 'history']
    ...
    <div className="card" style={{ maxWidth: 680, margin: '0 auto' }}>
      ...
      <form onSubmit={onSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
          ...
          <Input label="Budget (₹)" type="number" required placeholder="50000" value={form.budget} onChange={e => onChange({ budget: e.target.value })} />
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
            {GROUP_TYPES.map(g => (
              <button key={g} type="button" onClick={() => onChange({ groupType: g })}
                className={`btn btn-sm ${form.groupType === g ? 'btn-primary' : 'btn-secondary'}`}
        ...
        <div>
          <label style={{ fontSize: '0.875rem', fontWeight: 500, color: 'var(--color-text-secondary)', display: 'block', marginBottom: '0.5rem' }}>Preferences</label>
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
            {PREFERENCES.map(p => (
              <button key={p} type="button" onClick={() => togglePref(p)}
                className={`btn btn-sm ${prefs.includes(p) ? 'btn-primary' : 'btn-secondary'}`}
                style={{ textTransform: 'capitalize' }}>
                {p}
              </button>
            ))}
...
export default function Planner() {
  ...
  const { selections, activeDay, activeTab } = useSelector(selectPlanner)
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '1rem' }}>
                    {plan.transport_options.map((t, i) => (
                      <div key={i} onClick={() => dispatch(selectTransport(t))}
                        className="card card-hover" style={{ cursor: 'pointer', borderColor: selections.transport?.mode === t.mode ? 'var(--color-accent-primary)' : undefined, background: selections.transport?.mode === t.mode ? 'rgba(99,102,241,0.08)' : undefined }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '1rem' }}>
                    {plan.hotel_options.map((h, i) => (
                      <div key={i} onClick={() => dispatch(selectHotel(h))}
                        className="card card-hover" style={{ cursor: 'pointer', borderColor: selections.hotel?.name === h.name ? 'var(--color-accent-primary)' : undefined, background: selections.hotel?.name === h.name ? 'rgba(99,102,241,0.08)' : undefined }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '1rem' }}>
                    {plan.food_plans.map((f, i) => (
                      <div key={i} onClick={() => dispatch(selectFood(f))}
                        className="card card-hover" style={{ cursor: 'pointer', borderColor: selections.food?.name === f.name ? 'var(--color-accent-primary)' : undefined, background: selections.food?.name === f.name ? 'rgba(99,102,241,0.08)' : undefined }}>
                                ...
                                {day[slot].activities.map((act, j) => {
                                  ...
                                  return (
                                    <div key={j} onClick={() => dispatch(toggleActivity({ day: activeDay, slot, activity: act }))}
                                      className="card" style={{ cursor: 'pointer', padding: '0.875rem', borderColor: isSelected ? 'var(--color-accent-primary)' : undefined, background: isSelected ? 'rgba(99,102,241,0.08)' : 'rgba(255,255,255,0.02)' }}>
